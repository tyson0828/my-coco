...
def png2cocojson(dataDir, indent=None):
    '''
    Converts a folder of .png images with segmentation results back
    to the COCO result format. 
    ...
    dataDir2 = os.path.abspath(dataDir)
    fldName = os.path.basename(dataDir2)
    # Define paths
    pngFolder = '%s/labels' % (dataDir2)
    jsonPath = '%s/annotations/%s.json' % (dataDir2, fldName)
    ...
    with io.open(jsonPath, 'w', encoding='utf8') as output:
        ...
        for i, imgName in zip(range(0, imgCount), imgNames):
            ...
            print('pngPath %s, imgName %s' % (pngPath, imgName))
            anns = pngToCocoResult(pngPath, imgName, 0)
...
def main():
    ...
    parser.add_argument('--indent', nargs='?', type=int, default=-1)
    args = parser.parse_args()

    indent = (None if args.indent == -1 else args.indent)
    png2cocojson(args.data, indent)
